...
class YOLO_TF:
	...
	def build_networks(self):
		...
		if self.disp_console : print "net Loading complete!" + '\n'
#conv
#conv_layer conv_layer(1,self.x,64,7,2)
# conv_layer(3, self.pool_2, 192, 3, 1)  3*3 conv stride 1 canshu 3*3*196
    ...
    #fc self.fc_30 = self.fc_layer(30,self.fc_29,4096,flat=False,linear=False)
	...
	#4  network output 7 *7*2    bbox  pro and   20class so 7*7*(2*5+20)=1470
	def interpret_output(self,output):
		...
		#  each scales * class_pros  corresponding
		for i in range(2):
			for j in range(20):
				probs[:,:,i,j] = np.multiply(class_probs[:,:,j],scales[:,:,i])
		#  if >= probs
		filter_mat_probs = np.array(probs>=self.threshold,dtype='bool')
		# return nonzero location
		filter_mat_boxes = np.nonzero(filter_mat_probs)
		boxes_filtered = boxes[filter_mat_boxes[0],filter_mat_boxes[1],filter_mat_boxes[2]]
		# nonzero probs
		probs_filtered = probs[filter_mat_probs]
		# return max index class one of the 20
	...
	# 6 get results  xywh pro //show the result rectangle
